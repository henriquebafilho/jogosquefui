            ...
            <>
                <div className="container" style={{ color: Times(this.props.meuTime).letterColor, backgroundColor: Times(this.props.meuTime).backgroundColor }}>
                    {jogos.length > 0 ? jogos.map((index) => {
                        return <div>
                            <LinhaJogo key={JSON.stringify(index)} meuTime={meuTime} jogo={index} onSelectEstadio={this.props.onSelectEstadio} onSelectAdversario={this.props.onSelectAdversario} />
                        </div>
                    }) : <div>
